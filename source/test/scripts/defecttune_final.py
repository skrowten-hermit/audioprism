...
def TEST_CLICK():
    print "Click Detector parameter value range testing starting...."
    for APCDD in np.arange(APCDD_MIN, APCDD_MAX + 0.5, 0.5):
        ...
        for APCDP in range(APCDP_MIN, APCDP_MAX + 1):
            ...
            for APCDS in range(APCDS_MIN, APCDS_MAX + 1):
                ...
                for file in clikfiles:
                    exec_ap(file, params)
    print "Click Detector parameter value range testing completed...."


def TEST_NOISEBURST():
    print "Noise Burst parameter value range testing starting...."
    for APCDD in np.arange(APCDD_MIN, APCDD_MAX + 0.5, 0.5):
        print "Current APCDD : " + str(APCDD) + "\n"
        for APCDP in range(APCDP_MIN, APCDP_MAX + 1):
            print "Current APCDP for APCDD " + str(APCDD) + " : " + str(APCDP) + "\n"
            for APCDS in range(APCDS_MIN, APCDS_MAX + 1):
                ...
                for file in clikfiles:
                    exec_ap(file, params)
    print "Noise Burst parameter value range testing completed...."


def TEST_GAPS():
    print "Gaps Detector parameter value range testing starting...."
    for APCDD in np.arange(APCDD_MIN, APCDD_MAX + 0.5, 0.5):
        print "Current APCDD : " + str(APCDD) + "\n"
        for APCDP in range(APCDP_MIN, APCDP_MAX + 1):
            print "Current APCDP for APCDD " + str(APCDD) + " : " + str(APCDP) + "\n"
            for APCDS in range(APCDS_MIN, APCDS_MAX + 1):
                ...
                for file in clikfiles:
                    exec_ap(file, params)
    print "Gaps Detector parameter value range testing completed...."
...
if __name__ == "__main__":
    ...
    test_proc()
    print "Resetting to floor values...."
    setparmtomin()
    TEST_CLICK()
    setparmtomin()
    TEST_NOISEBURST()
