...
int main(int argc, char* argv[])
{
  ...
  while (true)
  {
    ...
    if (isSilent(NDframe)) continue;
    
    ndProb->compute();
  }

  pool.add("NoiseBurstDetector.Indexes", noiseburstIdxvec);

  cout << "Size of mono audio data of source : " << sampleSize << endl;
  cout << "Size of NoiseBurst indexes : " << noiseburstIdxvec.size() << endl;
